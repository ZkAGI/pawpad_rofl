...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // 3. Encrypt new TOTP secret for MongoDB storage
    const newEncryptedSecret = await encryptTotpSecret(uid, newSecret);

    // 4. Update MongoDB with new encrypted secret (CRITICAL FIX!)
    try {
      await UserConfig.findOneAndUpdate(
        { uid },
        { $set: { encryptedTotpSecret: newEncryptedSecret, updatedAt: new Date() } }
      );
      console.log(`Updated encrypted TOTP secret in MongoDB for user ${uid}`);
    } catch (e: any) {
      console.error("Failed to update MongoDB with new credentials:", e);
    ...
    // 5. Update On-Chain Policy (Trusted Signer)
    // This allows the user to gain trust from the contract again with new credentials
    try {
      await sapphireUpdateCommitments({
        uid,
